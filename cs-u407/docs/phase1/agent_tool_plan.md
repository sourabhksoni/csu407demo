...
### 3.5 Supporting tool: district and commodity normaliser
...
Phase 2 begins with a prompted base model as the baseline. The evaluation set is built and held
...
A LoRA or QLoRA adapter is then trained on our own extraction examples, weighted towards the
Hindi language and scanned document cases where prompting alone is weakest. Accuracy with and
without the adapter is reported on the same held out set. If the adapter does not beat the
baseline we report that result rather than adjusting the evaluation set to make it look better.
