...
| aliases | text[] | Every observed spelling, case variant and trade name mapping to this row |
...
First, naming. The same crop appears across our sources as `Rice`, `RICE`, `Paddy`, `paddy`
and `धान`, and a market source may report it under a trade grade rather than a crop name.
Without a canonical commodity row and an alias list, the same crop splits into several
entities and every aggregate silently undercounts.

Second, units. Procurement volumes are published in metric tonnes while prices are published
...
Pest and disease information is recorded here under `advisory_type = pest` rather than in a
separate table. It arrives in the same documents as weather and sowing advisories, carries the
same district and date keys, and would duplicate every column of this table. Adding a fifth
fact table for it would be schema churn without a corresponding gain.

### RemoteSensing (optional extension)

Included only if the Sentinel-2 indicator work proceeds. Nothing else in the schema depends on
it, so it can be dropped without affecting any other table or query.

| Field | Type | Notes |
|---|---|---|
| rs_id | PK | |
| location_id | FK | District level aggregate, never per pixel |
| commodity_id | FK | Nullable |
| observation_date | date | Acquisition date of the underlying scene |
| ndvi_mean | numeric | |
| ndvi_stddev | numeric | |
| ndre_mean | numeric | |
| evi_mean | numeric | |
| cloud_fraction | numeric | Quality flag, rows above a threshold are excluded from analysis |
| source_id | FK | |

Indicators are aggregated to district and date before being written, so what is stored is
tabular and joins on the same District plus Date key as every other fact table. No raster is
interpreted at query time and no image is stored in the database. This keeps the component
consistent with the brief's direction to treat spatial data as vector or tabular rather than
as raster interpretation.
...
- RemoteSensing, if built, is a derived indicator rather than a measurement of the quantity it
  is compared against. Spectral response is a proxy and is documented as one, not treated as an
  independent estimate of production.
