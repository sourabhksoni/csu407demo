...
| 3 | One pulse (chana or urad, fixed after source checking) | Traded on the open market with no administered floor, so it acts as the control case against the two procured crops |
...
The exact pulse is frozen once source coverage is confirmed. Other commodities remain in the
inventory as future work but have no fact rows in Phase 2.
