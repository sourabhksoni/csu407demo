...
src/
  ...
  tools/               api_csv, html_table, pdf_text, ocr
...
- 33 candidate sources reviewed: 24 verified live, 7 needing verification, 2
...
- Commodity scope frozen to four, including one control commodity with no
  administered price
- Evaluation harness and metrics implemented and tested
- District and commodity normalisation implemented and tested, 19 tests passing

## Open items

- Add Instructor in Charge (`Dajyu-20188`) and TA (`aniketqw`) as collaborators
- Complete team sign off on the course NDA and IP agreement
- Resolve the 7 sources marked as needing verification
- Confirm the district predecessor mapping in `src/normalisation/districts.csv`
  against the state gazette notifications before loading any pre 2022 data
- Confirm Agmarknet coverage for chana across Chhattisgarh mandis, since the
  control commodity depends on it
- Replace the two template records in `data/eval/extraction_eval.jsonl`
- Pull a sample file from at least five verified sources, record each hash in
  `data/manifest.csv`, and flip Sample pulled to Y for those rows only
- Locate the current Chhattisgarh Revenue Department agricultural statistics
  document from the department index; the URL in the earlier draft was malformed

## Note on source verification

Every candidate source was opened and checked against live results before being
included. Sources that could not be confirmed are retained in the inventory as
documented data gaps rather than deleted, because the Phase 1 acceptance criteria
ask for the limitations of current sources, not only a list of working links.
